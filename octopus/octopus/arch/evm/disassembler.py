...
class EvmDisassembler(Disassembler):
    ...
    def runtime_code_detector(self):
        ...
        '''
        result = list(re.finditer('6080604052', self.bytecode))
        if len(result) > 1:
    ...
    def swarm_hash_detector(self):
        ...
        # and not fake code
        swarm_hash_off = self.bytecode.rfind('a264697066735822')#('a165627a7a72')
        if( swarm_hash_off < 0 ):
            swarm_hash_off = self.bytecode.rfind('a165627a7a72305820') # bzzr 
        # bzzr == 0x65627a7a72
        if swarm_hash_off > 0:
            logging.info("[+] Swarm hash detected in bytecodes")
            swarm_hash = self.bytecode[swarm_hash_off:]
            logging.info("[+] Swarm hash value: 0x%s", swarm_hash)
            logging.info("[+] Swarm hash removed")
            self.swarm_hash = self.bytecode[swarm_hash_off:]
